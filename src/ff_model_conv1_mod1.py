...
class FF_model_conv1_mod1(torch.nn.Module):
    """The model trained with Forward-Forward (FF)."""
    """这个model是尝试解决layer_acc为0.5的假设1实验，控制每一层输出feature为3000维，用来与3072维全连接层表现对比"""

    def __init__(self, opt):
        super(FF_model_conv1, self).__init__()

        self.opt = opt
       self.num_channels=[30,120,750]
        self.num_strides=[1,1,1]
        self.receptions=[23,6,4]
        self.dim=[10,5,2]
