...
def train(opt, model, optimizer):
    ...
    train_loader = utils.get_data(opt, "train") #训练的dataloader
    num_steps_per_epoch = len(train_loader) #批次数

    for epoch in range(opt.training.epochs):
        train_results = defaultdict(float)
        optimizer = utils.update_learning_rate(optimizer, opt, epoch)   #每个epoch都cooldown lr
        ...
        if epoch % opt.training.val_idx == 0 and opt.training.val_idx != -1:    
            #-1: validate only once training has finished; n: validate every n epochs
